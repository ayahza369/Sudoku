...
}  //Pre: No specific prerequisites 
   //Post:returns a map that includes each unique substring of the sequence mapped to its frequency in the substring 
   public Map<String, Integer> generateFrequencies(int index){
      Map<String,Integer> myMap = new HashMap<String,Integer>();//New map made/initialized
      String sequence = sequences.get(index);//the specific sequence is fetched from the sequences string using the index
      for (int j=0; j<sequence.length(); j++){//iterating starting from the first character of the sequence
         for (int i =j+1; i<=sequence.length(); i++){//iterating across the letters that follow 
            if (myMap.containsKey(sequence.substring(j,i))){//if the substring from j to i is in the map
               myMap.put(sequence.substring(j,i),myMap.get(sequence.substring(j,i))+1);//its frequency is increased by 1
            } else {
               myMap.put(sequence.substring(j,i),1);//if not existent yet, it's added to the map
            }
         }
      }
      return myMap;//map returned
   }   
   //Pre: none
   //Post: returns a sorted List of the keys generated in the method generateFrequencies
   public List<String> getSortedListOfSubstrings(int index){
      Set<String> mySet = generateFrequencies(index).keySet();//creates a set of the sequence substrings as generated with generateFrequencies

      List<String> keysArray = new ArrayList<>();//new ArrayList created
      for (String key: mySet){//iterates through objects in the set
         keysArray.add(key);//adds each key from the set to the list
      }
      Collections.sort(keysArray);//sorts the list created
      return keysArray;//returns the sorted list
